...
func cpuBoundTask(id int, iterations int) {
	...
	fmt.Printf("CPU-bound task %d completed with sum: %d\n", id, sum)
}

func ioBoundTask(id int, duration time.Duration) {
	start := time.Now()
...
func contextAwareWorker(ctx context.Context, id int, work chan int, results chan int) {
	for {
		...
		case job, ok := <-work:
			if !ok {
				fmt.Printf("Worker %d stopping due to channel closure\n", id)
				return
			}
			// Simulate work
...
func main() {
	...
	for i := 0; i < 4; i++ {
		...
		go func(id int) {
			defer wg.Done()
			ioBoundTask(id, time.Millisecond*100)
		}(i)
	}
	wg.Wait()
